...
func (self *Pool) loopQ(s *slot) {
	...
	for {
		...
		s.task()
		s.task = nil
		// Park and publish availability after the runtime marks this G waiting.
		gopark(parkSlot, unsafe.Pointer(s), waitReasonZero, traceEvGoBlock, 1)
	}
}

func parkSlot(_ unsafe.Pointer, lock unsafe.Pointer) bool {
	s := (*slot)(lock)
	s.pool.push(s)
...
func (self *Pool) pop() (value *slot) {
	for {
		...
		if self.top.CompareAndSwap(top, top.next) {
			return top.value
		}
	}
}
